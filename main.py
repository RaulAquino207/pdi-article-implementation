...
import glob
import os
dataset_positive = 'dataset/posite'
dataset_negative = 'dataset/negative'

image_paths_positive = glob.glob(os.path.join(dataset_positive, '*.jpg'))
image_paths_negative = glob.glob(os.path.join(dataset_negative, '*.jpg'))

# ap = argparse.ArgumentParser(description='Running the argpaser')
...
# cv2.imshow('prototype', prototypeImg)
# cv2.waitKey(0)
...
buff1 = 10
(x, y, w, h) = maxBox
cv2.rectangle(targetImage, (int(x-buff1/2),int(y-buff1/2)),(int(x+w+buff1/2),int(y+h+buff1/2)), (255, 0, 0), 2)
